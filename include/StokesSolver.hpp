...
namespace NavierStokesPDE{
    ...
    template<int dim>
    class StokesSolver : public StokesParamHandler<dim>{
    ...
    template<int dim>
    double StokesSolver<dim>::setup(String parameter_filename) {
    ...
    template<int dim>
    double StokesSolver<dim>::assemble() {
        Timer timer;
        timer.start();
            ...
            for (unsigned int q=0; q<n_q; ++q) {
                for (unsigned int i=0; i<dofs_per_cell; ++i) {
                    for (unsigned int j=0; j<dofs_per_cell; ++j) {
                        ...
                        // + b(v,p) = (-∫ p⋅div(v) dx)
                        stiff_local(i,j) -= fe_v[vel].divergence(i,q) *
                                            fe_v[press].value(j,q) *
                                            fe_v.JxW(q);
                        // + b(u,q)
                        stiff_local(i,j) -= fe_v[vel].divergence(j,q) *
    ...
    template<int dim>
    void StokesSolver<dim>::output() const {
    ...
    /*
    template<int dim>
    double StokesSolver<dim>::compare_solution(
        const VectorTools::NormType &norm_type,
        const Function<dim> &exact
    ) const {
