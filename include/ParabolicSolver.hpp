...
namespace ParabolicPDE{
    ...
    double ParabolicSolver<dim>::assemble_rhs() {
        ...
        for (const auto & cell : dof_handler.active_cell_iterators()) {
           ...
           if (cell->at_boundary()) {
               for (unsigned int face_id = 0 ; face_id < cell->n_faces(); ++face_id) {
                   ...
                   bool is_neumann_boundary = std::find(this->boundary_tags.begin(),this->boundary_tags.end(),face->boundary_id()) == this->boundary_tags.end();
                   if (face->at_boundary() && is_neumann_boundary) {
                       ...
                       for (unsigned int q=0; q<boundary_quadrature->size(); ++q) {
                           ...
                           this->neumann_bc.set_time(curr_time);
                           auto curr_neumann = this->neumann_bc.value(x_q);
                           this->neumann_bc.set_time(curr_time-this->time_step);
                           auto old_neumann = this->neumann_bc.value(x_q);
        ...
        // for (auto tag : this->boundary_tags)
        //     boundary_functions[tag] = &this->dirichlet_bc;
        //
    ...
    double ParabolicSolver<dim>::run(const String &parameter_filename) {
        ...
        VectorTools::interpolate(dof_handler, this->initial_state, owned_sol);
