...
#include <fmt/format.h>
    ...
    #define LOG_TITLE(title) pcout << LOG_PREFIX << CYAN \
        << "================ " << fmt::format("{:^46}",title) << " ================" \
        << LOG_SUFFIX <<std::endl;
    #define LOG_VAR(name,val) pcout << LOG_PREFIX << GREEN << fmt::format("{:<90}",fmt::format("{}{} == {}{}",name, CYAN, YELLOW, val)) << CYAN << LOG_SUFFIX <<std::endl;
    #define LOG(val) pcout << LOG_PREFIX << YELLOW << fmt::format("{:<80}",val) << CYAN << LOG_SUFFIX <<std::endl;
    #define LOG_ANY(formt,arg) pcout << LOG_PREFIX << YELLOW << fmt::format("{:<80}",fmt::format(formt,arg)) << CYAN << LOG_SUFFIX <<std::endl;
#endif
